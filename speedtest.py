...
# All Rights Reserved.
# Modernized by SudeeptoBhakat — Python 3.8+, jitter, logging, HTTPS default
...
# Optional rich UI — install via: pip install speedtest-cli[ui]
try:
...
# HTTP error types for exception handling
try:
...
class SpeedtestHTTPConnection(HTTPConnection):
    """Custom HTTPConnection with source_address and timeout support."""
...
class SpeedtestHTTPSConnection(HTTPSConnection):
    """Custom HTTPSConnection with source_address and timeout support."""
...
def _build_connection(connection, source_address, timeout, context=None):
    """Build an HTTPConnection or HTTPSConnection with the args we need.
...
def build_opener(source_address=None, timeout=10):
    ...
    if DEBUG:
        print("Timeout set to %d" % timeout)

    if source_address:
        source_address_tuple = (source_address, 0)
        if DEBUG:
            print("Binding to source address: %r" % (source_address_tuple,))
    else:
...
def build_request(url, data=None, headers=None, bump="0", secure=False):
    ...
    if DEBUG:
        print("%s %s" % (("GET", "POST")[bool(data)], final_url))
...
class Speedtest:
    ...
    def get_config(self):
        ...
        request = build_request(
            "://www.speedtest.net/speedtest-config.php", headers=headers, secure=self._secure
        )
        ...
        try:
            ...
            raise SpeedtestConfigError(
                "Unknown location: lat=%r lon=%r" % (client.get("lat"), client.get("lon"))
            )
        ...
        for url in urls:
            try:
                ...
                for server in elements:
                    ...
                    try:
                        d = distance(
                            self.lat_lon, (float(attrib.get("lat")), float(attrib.get("lon")))
                        )
                    except Exception:
                        continue
    ...
    def get_best_server(self, servers=None):
        ...
        server has the lowest latency. Also calculates jitter.
        """
        ...
        for server in servers:
            ...
            # Calculate jitter (mean absolute deviation of consecutive pings)
            valid_pings = [c * 1000.0 for c in cum if c < 3600]
            if len(valid_pings) >= 2:
                diffs = [
                    abs(valid_pings[j] - valid_pings[j - 1]) for j in range(1, len(valid_pings))
                ]
                jitter_results[avg] = round(sum(diffs) / len(diffs), 3)
            else:
                jitter_results[avg] = 0.0
        ...
        self.results.jitter = jitter_results.get(fastest, 0.0)
    ...
    def upload(self, callback=do_nothing, pre_allocate=True, threads=None):
        ...
        for i, size in enumerate(sizes):
            ...
            data = HTTPUploaderData(
                size, 0, self.config["length"]["upload"], shutdown_event=self._shutdown_event
            )
            if pre_allocate:
                data.pre_allocate()
            ...
            requests.append(
                (build_request(self.best["url"], data, secure=self._secure, headers=headers), size)
            )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Only test latency and jitter, skip download and upload tests",
    )
    parser.add_argument(
        "--single",
        default=False,
    ...
    parser.add_argument(
        "--secure", action="store_true", default=True, help="Use HTTPS (default: enabled)"
    )
    parser.add_argument(
        "--no-secure", dest="secure", action="store_false", help="Use HTTP instead of HTTPS"
    )
    parser.add_argument(
        "--no-pre-allocate",
        dest="pre_allocate",
    ...
    parser.add_argument(
        ...
        nargs="?",
        metavar="FILE",
        const=os.path.expanduser("~/.speedtest-cli.log"),
        default=None,
        help="Append results as JSON to FILE (default: ~/.speedtest-cli.log)",
    )
    parser.add_argument("--version", action="store_true", help="Show the version number and exit")
    parser.add_argument("--debug", action="store_true", help=SUPPRESS, default=SUPPRESS)

    return parser.parse_args()


def validate_optional_args(args):
    """Validate that required modules are available for requested features."""
    pass  # json, ssl, HTTPSConnection always available in Python 3.8+
...
def shell():
    ...
    printer(
        "Hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: %(latency)s ms" % results.server, quiet
    )
    ...
    if args.download:
        ...
        if console:
            with Progress(
                ...
                speedtest.download(
                    callback=create_callback(progress, task), threads=(None, 1)[args.single]
                )
        else:
    ...
    if args.simple:
        print("Ping: %s ms" % results.ping)
        print("Jitter: %s ms" % results.jitter)
        if args.download:
            print(
                "Download: %0.2f M%s/s"
        ...
        print(results.csv(args.csv_delimiter))
    elif args.json:
        ...
        if console:
            ...
            table.add_row("Jitter", f"{results.jitter:.2f} ms")
            if args.download:
            ...
            printer("Jitter: %s ms" % results.jitter, quiet)
            if args.download:
                printer(
                    "Download: %0.2f M%s/s"
                    % ((results.download / 1000.0 / 1000.0) / args.units[1], args.units[0]),
    ...
    if args.log:
        ...
        printer("Results logged to %s" % args.log, quiet)
